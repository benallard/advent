...
fn main(){
    ...
    let numbers = lines[0].split(",").into_iter().map(|n| n.parse::<u32>().unwrap()).collect::<Vec<_>>();

    let mut boards = lines[1..].chunks(6).into_iter().map(|c| c[1..].join("\n").parse::<Board>().unwrap()).collect::<Vec<_>>();
...
impl Board{
    ...
    fn done(&self) -> bool{
        ...
        for i in 0..5{
            if self.board.iter().map(|l| l[i]).filter(|o| o.is_none()).count() == 5{
                return true;
            }
        }
        false
